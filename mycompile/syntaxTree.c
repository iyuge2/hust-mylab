...
GrammaNode* InsertNode(char* name,int num,...)
{
    ...
    if(num>0){//num>0为非终结符
        ...
        else if(!strcpy(newNode->name,"STRUCT")){
            strcpy(newNode->type,"struct");
        }
        else{
            if(!strcmp(preword,"int")){
                strcpy(newNode->type,"int");
                newNode->tag = 1;
            }
            else if(!strcmp(preword,"float")){
                strcpy(newNode->type,"float");
                newNode->tag = 1;
            }
            else if(!strcmp(preword,"struct")){
                strcpy(newNode->type,"struct");
                newNode->tag = 5;
            }
            strcpy(newNode->content,yytext);
        }
        strcpy(preword,yytext);
    }
    return newNode;
}
